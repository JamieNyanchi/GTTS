...
local function updateSurfaces()
    for name, surface in pairs(game.surfaces) do
        update_GTTS_setting("gtts-Adjust-DayNight", "initial-day-night_" .. name,
            { "surfaces", name, "ticks_per_day" }, false)
        update_GTTS_setting("gtts-Adjust-WindSpeed", "initial-windspeed_" .. name,
            { "surfaces", name, "wind_speed" }, true)
    end
end
...
local function updateMapSettings()
    ...
    update_GTTS_setting("gtts-Adjust-Pollution", "initial-pollution-ageing",
        { "map_settings", "pollution", "ageing" }, true)
...
if settings.startup["gtts-z-No-Runtime-Adjustments"].value == false then
    map_adjuster.on_configuration_changed = on_configuration_changed

    map_adjuster.events = {
        [defines.events.on_tick] = on_tick,
        [defines.events.on_runtime_mod_setting_changed] = on_runtime_mod_setting_changed,
        [defines.events.on_surface_created] = updateSurfaces,
    }
end
